...
func attack(body: Node3D, rot_node: Node3D, dmg: float, ignore_armor: bool, remote_peer: int, push_velocity := 1.0, damage_types := {"melee":1.0}) -> void:
	# Урон по игрокам
	if body.is_in_group("players"):
		body.health.take_damage.rpc_id(int(body.name), dmg, ignore_armor, remote_peer, damage_types)
	...
	if body.is_in_group("objects") or body.is_in_group("buildings"):
		body.health.take_damage.rpc_id(1, dmg, ignore_armor, remote_peer, damage_types)
	
	if body.is_in_group("sub_blocks"):
		body.get_parent().health.take_damage.rpc_id(1, dmg, ignore_armor, remote_peer, damage_types)
